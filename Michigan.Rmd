---
title: "Michigan"
author: "Jordan Burton, mahzad, Ameen"
...
ggplot(TotalMIAcc, aes(x = County, y = Accidents, col = log10(TotalPop), fill = log10(TotalPop))) + # This line is used to determine what data will be used to fill the plots
...
ggplot(MI_county_map_acc, aes(long, lat, group=group)) + # group = group is required to keep all of the county longitudes and latitudes together per county
  geom_polygon(aes(fill = log10(Accidents)), color = "white") +#geom_polygon allows the map representation to be placed onto the graph
...
***Figure*** Yearly accidents in each Michigan County from 2016 - 2019.

```{r}

ggplot(MI_Sev_Co_Map, aes(long, lat, group=group)) +
  geom_polygon(aes(fill=log10(Accidents)), color = "white") + #, color = "white"
...
ggplot(MI_county_map_acc, aes(long, lat, group=group)) +
  geom_polygon(aes(fill = AccidentsPerCapita), color = "white") +
  theme_classic() +
...
ggplot(temperature, aes(Temp, Accidents)) + 
...
***Figure*** Three cities (Flint, Detroit, and Grand Rapids) in Michigan account for the majority of the accidents reported in the state from 2016-2019.
...
ggplot(Wayne_accidents, aes(Start_Lng, Start_Lat, fill = Street, col = Street)) +
  geom_point() +
  theme(legend.position = "none")

qmap("wayne county", zoom = 10) +
  geom_line(aes(Start_Lng, Start_Lat, fill = Street, col = Street), data = Wayne_accidents) +
  theme(legend.position = "none")
```
...
#  theme(legend.position = "none")
qmap("flint", zoom = 12) +
  geom_line(aes(Start_Lng, Start_Lat, fill = Street, col = Street), data = Genesee_accidents) +
  theme(legend.position = "none")

exit <- "I-75, M118, Flint"
qmap(exit, zoom = 16) +
    geom_point(aes(Start_Lng, Start_Lat, fill = Street, col = Street), data = Genesee_accidents) +
  theme(legend.position = "none")
```
...
ggplot(Kent_accidents, aes(Start_Lng, Start_Lat, col = Street, fill = Street)) +
  geom_point() + 
  theme(legend.position =  "none")

qmap("grand rapids", zoom = 12) +
  geom_line(aes(Start_Lng, Start_Lat, fill = Street, col = Street), data = Kent_accidents) +
  theme(legend.position = "none")
```
...
ggplot(Seasons, aes(x=Severity, y = log(Accidents), fill = Month)) +
